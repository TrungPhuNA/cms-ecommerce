                ...
                <div class="card-body">
                    <div class="form-group {{ $errors->has('t_name') ? 'was-validated' : '' }} ">
                        ...
                        <input type="text" class="form-control keypress-count" {{ $errors->has('t_name') ? 'required' : '' }}  data-add-content=".title_seo"
                               data-add-slug=".slug"
                               maxlength="120" name="t_name" placeholder="Name ..." value="{{ old('t_name',$menu->t_name ?? '') }}">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="t_slug" class="form-control slug" placeholder="Slug ..." value="{{ old('t_slug',$menu->t_slug ?? '') }}">
                    </div>
                    ...
                    <div class="form-group {{ $errors->has('t_description') ? 'was-validated' : '' }} ">
                        ...
                        data-max="300" class="form-control keypress-count"  cols="10" rows="3">{{ old('t_description', $menu->t_description ?? '') }}</textarea>
                    </div>
                </div>
            </div>
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="mg-b-5 existed-seo-meta mg-t-20">
                        <span class="page-title-seo title_seo">{{ $menu->t_title_seo ?? 'Title SEO' }} </span>
                        <div class="page-url-seo ws-nm">
                            <p>http://cms-laravel.net/bai-viet/<span class="slug">{{ ($menu->t_slug ?? '') }}</span></p>
                        </div>
                        <div class="ws-nm">
                            @if (isset($menu))
                            ...
                            @endif
                            <span class="page-description-seo description_seo">{{ $menu->t_description_seo ?? "" }}</span>
                        </div>
                    </div>
                    <div class="content-seo hide">
                        <div class="form-group">
                            ...
                            <small class="charcounter" data-max="120">(120 character(s) remain)</small>
                            <input type="text" class="form-control keypress-count title_seo " value="{{ old('t_title_seo', $menu->t_title_seo ?? '') }}" name="t_title_seo" placeholder="Name ...">
                        </div>
                        <div class="form-group">
                            ...
                            <textarea name="t_description_seo" class="form-control keypress-count description_seo" cols="10" rows="3">
                                {{ old('t_description_seo', $menu->t_description_seo ?? "") }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="SumoSelect sumo_somename" tabindex="0" role="button" aria-expanded="true">
                            <select name="t_status" class="form-control SlectBox SumoUnder" tabindex="-1">
                                <option title="3" value="3" {{ ($menu->t_status ?? '' ) == 3 ? "selected='selected'" : "" }}>Published</option>
                                <option value="2" {{ ($menu->t_status ?? '' ) == 2 ? "selected='selected'" : "" }}>Draft</option>
                                <option value="1" {{ ($menu->t_status ?? '' ) == 1 ? "selected='selected'" : "" }}>Pending</option>
                            </select>
                        </div>
                    </div>
                </div>
